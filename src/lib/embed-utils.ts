...
import type { PostEmbed, PostImage, PostVideo, PostExternal, PostAuthor, QuotedPost, RichLinkFacet } from '../types';

/**
 * Extract link facets with position info from ATproto richtext facets.
 * Facets contain the actual resolved URLs plus byte offsets for highlighting.
 */
export function extractLinkFacets(facets: unknown): RichLinkFacet[] {
  if (!Array.isArray(facets)) return [];
  const links: RichLinkFacet[] = [];
  for (const facet of facets) {
    ...
    for (const feature of f.features) {
      if (feature.$type === 'app.bsky.richtext.facet#link' && feature.uri) {
        links.push({
          url: feature.uri,
          byteStart: f.index?.byteStart ?? 0,
          byteEnd: f.index?.byteEnd ?? 0,
        });
      }
    }
  }
