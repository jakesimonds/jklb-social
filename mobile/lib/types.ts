// TypeScript type definitions for russAbbot mobile
// Copied from src/types/index.ts — pure TypeScript, 100% portable
 ...
 * Used for credible exit feature to select "best post" from session
 */
...
export interface Session {
  ...
  likedPosts?: LikedPost[]; // Posts liked during this session (for credible exit)
}

/**
 * Settings Types
 * All user-configurable settings stored in AsyncStorage
 */
export interface FeedSettings {
  ...
  threadViewMode: 'scroll';  // Thread viewer: vertical scroll list (ScrollThread)
}
...
export interface MusicSettings {
  ...
  selectedTrackUri: string | null;  // AT URI of the selected fm.plyr.track, null = none
}
...
export interface QuotedPost {
  ...
  video?: PostVideo;
  external?: PostExternal;
  /** URLs extracted from richtext link facets */
  linkFacets?: string[];
}
...
/**
 * Undo System Types
 * Used for tracking recent user actions that can be reversed
 */
export interface UndoableAction {
  id: string;
  type: 'like' | 'boost' | 'reply' | 'quote';
  timestamp: number;
  targetPost: {
    uri: string;
    cid: string;
...
/**
 * AsyncStorage Keys (adapted from localStorage keys for React Native)
 */
export const STORAGE_KEYS = {
  ...
  LAST_HANDLE: 'russabbot_last_handle',
} as const;
